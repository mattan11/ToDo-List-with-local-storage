...
function renderTaskCount(selectedList) {
  const incompleTasksCount = selectedList.tasks.filter((task) => !task.complete)
    .length;

  const taskString = incompleTasksCount === 1 ? "task" : "tasks";
  listCountElement.innerText = `${incompleTasksCount} ${taskString} remaining`;
}
